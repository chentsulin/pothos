...
export default class ConfigStore<Types extends SchemaTypes> {
    ...
    private fieldRefCallbacks = new Map<GenericFieldRef | GenericInputFieldRef, ((config: PothosFieldConfig<Types>) => void)[]>();
    ...
    addFieldRef(ref: ArgumentRef<Types> | FieldRef<Types> | InputFieldRef<Types>, 
    ...
    getImplementers(ref: ConfigurableRef<Types> | string) {
        ...
        const implementers = [...this.typeConfigs.values()].filter((type) => type.kind === "Object" &&
            type.interfaces.find((i) => this.getTypeConfig(i).name === typeConfig.name)) as PothosObjectTypeConfig[];
        return implementers;
    }
    ...
    private buildField(typeRef: ConfigurableRef<Types>, field: ArgumentRef<Types> | FieldRef<Types> | InputFieldRef<Types>, fieldName: string) {
