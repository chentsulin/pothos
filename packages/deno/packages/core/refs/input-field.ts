...
export class InputFieldRef<Types extends SchemaTypes, T = unknown> {
    ...
    private initConfig: ((name: string, typeConfig: PothosTypeConfig) => PothosInputFieldConfig<Types>) | null;
    private onUseCallbacks = new Set<(config: PothosInputFieldConfig<Types>) => void>();
    constructor(initConfig: ((name: string, typeConfig: PothosTypeConfig) => PothosInputFieldConfig<Types>) | null) {
